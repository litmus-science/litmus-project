...
@pytest.fixture
def sample_intake(examples_dir):
...
@pytest.fixture
def individual_lab(examples_dir):
...
@pytest.fixture
def commercial_lab(examples_dir):
...
@pytest.fixture
def bsl2_intake(minimal_intake):
...
class TestHardFilters:
    ...
    def test_experiment_type_filter_pass(self, sample_intake, commercial_lab):
    ...
    def test_experiment_type_filter_fail(self, sample_intake, individual_lab):
    ...
    def test_bsl_level_filter_pass(self, bsl2_intake, commercial_lab):
    ...
    def test_bsl_level_filter_fail(self, bsl2_intake, individual_lab):
    ...
    def test_human_material_filter_pass(self, minimal_intake, commercial_lab):
        """Lab approving human materials should pass when required."""
        intake = minimal_intake.copy()
        intake["compliance"]["human_derived_material"] = True
        lab = commercial_lab.copy()
        lab["compliance"]["human_samples_approved"] = True
        reasons = apply_hard_filters(intake, lab)
        assert FilterReason.HUMAN_SAMPLES_NOT_APPROVED not in reasons

    def test_human_material_filter_fail(self, minimal_intake, individual_lab):
    ...
    def test_animal_material_filter_fail(self, minimal_intake, individual_lab):
        """Lab not approving animal materials should fail when required."""
        intake = minimal_intake.copy()
        intake["compliance"]["animal_derived_material"] = True
        reasons = apply_hard_filters(intake, individual_lab)
        assert FilterReason.ANIMAL_SAMPLES_NOT_APPROVED in reasons

    def test_hazmat_filter_pass(self, sample_intake, commercial_lab):
    ...
    def test_inactive_lab_filtered(self, sample_intake, commercial_lab):
    ...
    def test_strict_deliverables_filter(self, sample_intake, individual_lab):
...
class TestDeliverablesGaps:
    ...
    def test_no_gaps_when_all_supported(self, sample_intake, commercial_lab):
        """No gaps when lab supports all required deliverables."""
        gaps = check_deliverables_gaps(sample_intake, commercial_lab)
        assert len(gaps) == 0

    def test_gap_for_missing_raw_format(self, sample_intake, commercial_lab):
        """Gap reported for unsupported raw format."""
        intake = sample_intake.copy()
        intake["deliverables"] = intake["deliverables"].copy()
        intake["deliverables"]["raw_data_formats"] = ["FASTQ"]  # Unlikely to be supported
        gaps = check_deliverables_gaps(intake, commercial_lab)
        assert "raw_format:FASTQ" in gaps

    def test_gap_for_missing_processed_output(self, sample_intake, individual_lab):
    ...
    def test_gap_for_package_level(self, minimal_intake, individual_lab):
...
class TestSpecCompleteness:
    ...
    def test_completeness_in_valid_range(self, sample_intake, minimal_intake):
    ...
    def test_empty_intake_low_score(self):
        """Empty/near-empty intake should score very low."""
        score = compute_spec_completeness({})
        assert score < 0.3

    def test_acceptance_criteria_bonus(self, minimal_intake):
...
class TestScoring:
    ...
    def test_default_weights_sum_approximately_one(self):
        ...
        w = DEFAULT_WEIGHTS
        total = (w.menu_fit + w.turnaround_fit + w.spec_completeness +
                 w.cost_fit + w.quality + w.logistics + w.deliverables_match)
        assert 0.95 <= total <= 1.05

    def test_score_components_non_negative(self, sample_intake, commercial_lab):
    ...
    def test_total_score_in_valid_range(self, sample_intake, commercial_lab):
    ...
    def test_good_lab_scores_higher(self, sample_intake, commercial_lab):
    ...
    def test_quality_flags_set(self, sample_intake, commercial_lab):
    ...
    def test_budget_exceeds_flags(self, minimal_intake, commercial_lab):
...
class TestRouteIntake:
    ...
    def test_route_returns_sorted_matches(self, sample_intake, commercial_lab):
        ...
        lower_lab["quality_metrics"]["average_rating"] = 3.5
    ...
    def test_route_filters_incompatible_labs(self, sample_intake, individual_lab, commercial_lab):
        ...
        assert individual_lab["lab_id"] not in match_ids
        assert individual_lab["lab_id"] in result.filtered_out

    def test_route_includes_filter_reasons(self, sample_intake, individual_lab):
        """Result should include reasons why labs were filtered."""
        labs = [individual_lab]
        result = route_intake(sample_intake, labs)

        lab_id = individual_lab["lab_id"]
        assert lab_id in result.filtered_out
        assert "experiment_type_not_supported" in result.filtered_out[lab_id]

    def test_route_respects_top_k(self, sample_intake, commercial_lab):
    ...
    def test_route_with_mic_intake(self, mic_intake, individual_lab):
    ...
    def test_route_includes_pricing_info(self, mic_intake, individual_lab):
    ...
    def test_route_includes_tat_info(self, mic_intake, individual_lab):
    ...
    def test_strict_deliverables_mode(self, sample_intake, commercial_lab):
        """Strict deliverables mode should filter labs that can't meet all requirements."""
        # Modify intake to require something unusual
        intake = sample_intake.copy()
        intake["deliverables"] = intake["deliverables"].copy()
        intake["deliverables"]["raw_data_formats"] = ["FASTQ"]  # Unlikely supported
    ...
    def test_region_preference_bonus(self, sample_intake, commercial_lab):
...
class TestValidation:
    ...
    def test_valid_intake_passes(self, sample_intake):
    ...
    def test_missing_hypothesis_statement_fails(self):
        ...
        assert not valid
        assert any("hypothesis" in e["path"] for e in errors)
    ...
    def test_missing_budget_fails(self):
        ...
        valid, errors, warnings = validate_intake(intake)
        assert not valid
        assert any("budget" in e["path"] for e in errors)
    ...
    def test_invalid_experiment_type_fails(self):
        ...
        valid, errors, warnings = validate_intake(intake)
        assert not valid
        assert any("experiment_type" in e["path"] for e in errors)

    def test_missing_null_hypothesis_warns(self, minimal_intake):
        """Missing null hypothesis should generate warning."""
        valid, errors, warnings = validate_intake(minimal_intake)
        assert any("null_hypothesis" in w["path"] for w in warnings)

    def test_low_completeness_warns(self):
        ...
        assert any("completeness" in w["message"].lower() for w in warnings)
...
class TestCustomWeights:
    ...
    def test_custom_weights_affect_scoring(self, sample_intake, commercial_lab):
...
class TestEdgeCases:
    ...
    def test_lab_missing_optional_fields(self, mic_intake):
