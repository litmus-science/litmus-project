...
class LabMatch:
    ...
    pricing_band_usd: dict[str, float | None] | None = None
...
def apply_hard_filters(
    ...
    required_shipping_mode: str | None = None,
    ...
    lab_caps = _as_object(lab.get("capabilities"))
    lab_compliance = _as_object(lab.get("compliance"))
    lab_logistics = _as_object(lab.get("logistics"))
    intake_compliance = _as_object(intake.get("compliance"))

    # 1. Experiment type support
    supported_types = _as_str_list(lab_caps.get("experiment_types"))
    ...
    intake_bsl = _as_str(intake_compliance.get("bsl")) or "BSL1"
    lab_max_bsl = _as_str(lab_compliance.get("max_bsl")) or "BSL1"
    if bsl_order.get(intake_bsl, 1) > bsl_order.get(lab_max_bsl, 1):
        reasons.append(FilterReason.BSL_EXCEEDED)

    # 3. Human/animal samples
    if (
        intake_compliance.get("human_derived_material") is True
        and lab_compliance.get("human_samples_approved") is not True
    ):
        reasons.append(FilterReason.HUMAN_SAMPLES_NOT_APPROVED)

    if (
        intake_compliance.get("animal_derived_material") is True
        and lab_compliance.get("animal_samples_approved") is not True
    ):
        reasons.append(FilterReason.ANIMAL_SAMPLES_NOT_APPROVED)

    # 4. Hazardous materials
    if (
        intake_compliance.get("hazardous_chemicals") is True
        and lab_compliance.get("hazardous_chemicals_approved") is not True
    ):
        reasons.append(FilterReason.HAZMAT_NOT_APPROVED)

    # 5. Shipping mode
    if required_shipping_mode:
        accepted_modes = _as_str_list(lab_logistics.get("shipping_modes_accepted"))
        if not accepted_modes:
            accepted_modes = ["AMBIENT"]
...
def check_deliverables_gaps(intake: JsonObject, lab: JsonObject) -> list[str]:
    """
    ...
    intake_deliverables = _as_object(intake.get("deliverables"))
    lab_deliverables = _as_object(lab.get("deliverables_support"))
    exp_type = _as_str(intake.get("experiment_type"))

    # Get lab's capabilities (merge global + experiment-specific)
    lab_global = _as_object(lab_deliverables.get("global"))
    lab_by_type = _as_object(lab_deliverables.get("by_experiment_type"))
    lab_specific = _as_object(lab_by_type.get(exp_type)) if exp_type else {}

    lab_raw_formats = set(_as_str_list(lab_global.get("raw_formats"))) | set(
        _as_str_list(lab_specific.get("raw_formats"))
    )
    lab_processed = set(_as_str_list(lab_global.get("processed_outputs"))) | set(
        _as_str_list(lab_specific.get("processed_outputs"))
    )

    # Determine lab's max package level
    level_order = {"L0_RAW_ONLY": 0, "L1_BASIC_QC": 1, "L2_INTERPRETATION": 2}
    lab_max_level_value = lab_specific.get("max_package_level")
    lab_max_level = _as_str(lab_max_level_value) or _as_str(lab_global.get("max_package_level"))
    lab_max_level_num = level_order.get(lab_max_level or "L0_RAW_ONLY", 0)

    # Check required raw formats
    required_raw = _as_str_list(intake_deliverables.get("raw_data_formats"))
    for fmt in required_raw:
        if fmt not in lab_raw_formats:
            gaps.append(f"raw_format:{fmt}")

    # Check required processed outputs
    required_processed = _as_str_list(intake_deliverables.get("required_processed_outputs"))
    ...
    required_level = _as_str(intake_deliverables.get("minimum_package_level")) or "L0_RAW_ONLY"
...
def compute_spec_completeness(intake: JsonObject) -> float:
    """
    ...
    section_key = type_sections.get(exp_type) if exp_type else None
    if section_key:
        max_score += 3.0
        section = _as_object(intake.get(section_key))
        if section:
    ...
    max_score += 1.0
    if _as_object(intake.get("acceptance_criteria")).get("success_conditions"):
        score += 1.0

    return score / max_score if max_score > 0 else 0.0


def get_nested(d: JsonObject, path: str) -> JsonValue | None:
    """Get nested dictionary value by dot-separated path."""
    keys = path.split(".")
    val: JsonValue | None = d
    for key in keys:
...
def score_lab(
    ...
    menu_score = 0.0
    lab_caps = _as_object(lab.get("capabilities"))
    lab_menu_tags = set(_as_str_list(lab_caps.get("menu_tags")))
    ...
    exp_type_key = exp_type if exp_type is not None else ""
    relevant_tags = exp_type_tags.get(exp_type_key, [])
    if relevant_tags:
        matches = len(lab_menu_tags & set(relevant_tags))
        menu_score = matches / len(relevant_tags)
    else:
        exp_types_supported = _as_str_list(lab_caps.get("experiment_types"))
        menu_score = 1.0 if exp_type and exp_type in exp_types_supported else 0.0

    breakdown["menu_fit"] = menu_score * weights.menu_fit

    # 2. Turnaround fit
    tat_score = 0.5  # Default to medium
    turnaround_budget = _as_object(intake.get("turnaround_budget"))
    desired_tat = _as_float(turnaround_budget.get("desired_turnaround_days"))
    commercial_terms = _as_object(lab.get("commercial_terms"))
    turnaround_days = _as_object(commercial_terms.get("turnaround_days"))
    lab_tat = _as_object(turnaround_days.get(exp_type)) if exp_type else {}

    if desired_tat is not None and lab_tat:
        lab_standard_tat = _as_float(lab_tat.get("standard")) or 14.0
    ...
    # 4. Cost fit
    cost_score = 0.5
    budget_max = _as_float(turnaround_budget.get("budget_max_usd"))
    pricing_bands = _as_object(commercial_terms.get("pricing_bands"))
    pricing = _as_object(pricing_bands.get(exp_type)) if exp_type else {}

    if budget_max is not None and pricing:
        typical = _as_float(pricing.get("typical_usd"))
        if typical is None:
            typical = _as_float(pricing.get("min_usd")) or 0.0
    ...
    # 5. Quality metrics
    quality = _as_object(lab.get("quality_metrics"))
    quality_score = 0.5

    if quality:
        components = []
        on_time_rate = _as_float(quality.get("on_time_rate"))
        if on_time_rate is not None:
            components.append(on_time_rate)
        avg_rating = _as_float(quality.get("average_rating"))
        if avg_rating is not None:
            components.append((avg_rating - 1) / 4)  # Normalize 1-5 to 0-1
        rerun_rate = _as_float(quality.get("rerun_rate"))
        if rerun_rate is not None:
            components.append(1 - rerun_rate)  # Lower is better
        data_pkg_score = _as_float(quality.get("data_package_score"))
        if data_pkg_score is not None:
            components.append(data_pkg_score)

        if components:
            quality_score = sum(components) / len(components)

        # Flag quality concerns
        rerun_rate_flag = _as_float(quality.get("rerun_rate")) or 0.0
        if rerun_rate_flag > 0.15:
            flags.append("elevated_rerun_rate")
        avg_rating_flag = _as_float(quality.get("average_rating")) or 5.0
        if avg_rating_flag < 4.0:
    ...
    logistics = _as_object(lab.get("logistics"))
    availability = _as_object(lab.get("availability"))

    capacity = _as_str(availability.get("current_capacity"))
    if capacity == "high":
        logistics_score = 1.0
    elif capacity == "low":
        logistics_score = 0.4
    ...
    if logistics.get("weekend_receiving") is True:
...
def route_intake(
    ...
    lab_ids = [_as_str(lab.get("lab_id")) for lab in labs if _as_str(lab.get("lab_id"))]
    ...
    for lab in labs:
        lab_id = _as_str(lab.get("lab_id")) or "unknown"
        lab_name = _as_str(lab.get("name")) or "Unknown Lab"
        ...
        if region_preference and _as_str(lab.get("region")) == region_preference:
            total_score *= 1.1  # 10% bonus for preferred region
            breakdown["region_bonus"] = total_score * 0.1

        # Get estimated TAT and pricing
        commercial = _as_object(lab.get("commercial_terms"))
        tat_info = (
            _as_object(_as_object(commercial.get("turnaround_days")).get(exp_type))
            if exp_type
            else {}
        )
        pricing_info = (
            _as_object(_as_object(commercial.get("pricing_bands")).get(exp_type))
            if exp_type
            else {}
        )

        tat_standard = _as_float(tat_info.get("standard"))
        ...
        match = LabMatch(
            ...
            estimated_tat_days=int(tat_standard) if tat_standard is not None else None,
            pricing_band_usd=(
                {
                    "min": _as_float(pricing_info.get("min_usd")),
                    "max": _as_float(pricing_info.get("max_usd")),
                }
                if pricing_info
                else None
            ),
...
def validate_intake(intake: JsonObject) -> tuple[bool, list[JsonObject], list[JsonObject]]:
    """
    ...
    if exp_type and exp_type not in valid_types:
        errors.append(
            {"path": "/experiment_type", "message": f"Invalid experiment type: {exp_type}"}
        )
            ...
            errors.append(
                {
                    "path": f"/{section_key}",
                    "message": (
                        f"Section '{section_key}' is required for experiment type {exp_type}"
                    ),
                }
            )
    ...
    if not _as_str(hypothesis.get("statement")):
        errors.append(
            {"path": "/hypothesis/statement", "message": "Hypothesis statement is required"}
        )
    if not _as_str(hypothesis.get("null_hypothesis")):
        warnings.append(
            {"path": "/hypothesis/null_hypothesis", "message": "Null hypothesis is recommended"}
        )

    # Budget validation
    budget = _as_object(intake.get("turnaround_budget"))
    if _as_float(budget.get("budget_max_usd")) is None:
        errors.append(
            {"path": "/turnaround_budget/budget_max_usd", "message": "Maximum budget is required"}
        )
        ...
        warnings.append(
            {
                "path": "/",
                "message": (
                    f"Intake completeness is low ({completeness:.0%}). "
...
if __name__ == "__main__":
    ...
    example_intake: JsonObject = {
        ...
        "compliance": {"bsl": "BSL1", "hazardous_chemicals": True},
        "turnaround_budget": {"desired_turnaround_days": 14, "budget_max_usd": 500},
        "deliverables": {
    ...
    example_labs: list[JsonObject] = [
        {
            ...
            "compliance": {"max_bsl": "BSL2", "hazardous_chemicals_approved": True},
            "commercial_terms": {
                "pricing_bands": {
                    "CELL_VIABILITY_IC50": {"min_usd": 300, "max_usd": 600, "typical_usd": 400}
                },
                "turnaround_days": {"CELL_VIABILITY_IC50": {"standard": 10, "expedited": 5}},
            },
            "quality_metrics": {
                "on_time_rate": 0.95,
                "average_rating": 4.8,
        ...
        },
        {
            ...
            "compliance": {"max_bsl": "BSL1", "hazardous_chemicals_approved": True},
            "commercial_terms": {
                "pricing_bands": {
                    "CELL_VIABILITY_IC50": {"min_usd": 200, "max_usd": 400, "typical_usd": 280}
                },
                "turnaround_days": {"CELL_VIABILITY_IC50": {"standard": 14, "expedited": 7}},
            },
            "quality_metrics": {"on_time_rate": 0.88, "average_rating": 4.2, "rerun_rate": 0.08},
            "availability": {"current_capacity": "medium"},
            "deliverables_support": {
                "global": {"raw_formats": ["CSV", "XLSX"], "max_package_level": "L1_BASIC_QC"}
            },
        },
    ]
